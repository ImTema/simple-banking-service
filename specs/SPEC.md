...
- **`Account`**: `id: AccountId` (UUID, auto-generated), `ownerId: OwnerId` (value class wrapping a non-blank `String`), mutable `balance: Money`, and its own `ReentrantLock` used to guard mutation. One account per owner (multiple accounts per user is out of scope; no uniqueness enforcement needed beyond UUID generation).
